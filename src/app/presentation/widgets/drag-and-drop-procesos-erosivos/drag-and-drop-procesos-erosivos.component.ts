...
@Component({
  selector: 'cuenca-drag-and-drop-procesos-erosivos',
  templateUrl: './drag-and-drop-procesos-erosivos.component.html',
  styleUrls: ['./drag-and-drop-procesos-erosivos.component.css']
})
export class DragAndDropProcesosErosivosComponent extends BaseComponent {
  ...
  uploadFiles(): void {
    ...
    if (this.fileList.length > 0) {
      ...
      Array.from(this.fileList).forEach((file: File) => {
        this.tasksManager.makeFileRequestBaseDocumento(file, '3')
          .then((response: any) => {
