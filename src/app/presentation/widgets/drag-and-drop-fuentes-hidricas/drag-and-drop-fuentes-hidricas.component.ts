...
@Component({
  selector: 'cuenca-drag-and-drop-fuentes-hidricas',
  templateUrl: './drag-and-drop-fuentes-hidricas.component.html',
  styleUrls: ['./drag-and-drop-fuentes-hidricas.component.css']
})
export class DragAndDropFuentesHidricasComponent extends BaseComponent {
  ...
  uploadFiles(): void {
    ...
    if (this.fileList.length > 0) {
      ...
      Array.from(this.fileList).forEach((file: File) => {
        this.tasksManager.makeFileRequestBaseDocumento(file, '2')
          .then((response: any) => {
