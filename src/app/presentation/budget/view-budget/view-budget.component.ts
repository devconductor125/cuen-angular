...
export class ViewBudgetComponent extends BaseComponent implements OnInit {
  ...
  public totalAccionesDeRestauracionActiva: number;
  public totalAccionesDeRestauracionPasiva: number;
  public totalBuenasPracticasYSaneamiento: number;
  ...
  constructor(protected messagingService: MessagingService,
    ...
    this.totalAccionesDeRestauracionActiva = 0;
    this.totalAccionesDeRestauracionPasiva = 0;
    this.totalBuenasPracticasYSaneamiento = 0;
  }
  ...
  protected loadProject() {
    ...
    this.activatedRoute.paramMap
      ...
      .subscribe(idString => {
        ...
        this.tasksManager.getBudgetFromProcess(id)
          .then(function (budget: any) {
            component.budgets = budget;
            component.filterByZona(budget.conservation_area);
          });
      });
  }

  private filterByZona(data: Array<any>): void {
    let totalAreas = 0;
    ...
    const proposedAreasDataNacimientos = data.filter(action => action.action.includes('nacimiento'));
    const proposedAreasDataRibera = data.filter( action => action.action.includes('ribera'));
    const proposedAreasDataLadera = data.filter( action => action.action.includes('ladera'));
    ...
    totalAreas += proposedAreasDataNacimientosArea;
  ...
  /// añadir aporte a cada tipo de actividad se hizo por separado para facilitar el manejo
  public addAporteUno(): void {
    ...
    if (this.selectAssociatedUno === '0' || !this.montoUno) {
      ...
      const objeto = {
        ...
        budget_id: this.selectAssociatedUno.budget_id
      };
  ...
  public addAporteTres(): void {
    ...
    if (this.selectAssociatedTres === '0' || !this.montoTres) {
      ...
      const objeto = {
        ...
        budget_id: this.selectAssociatedTres.budget_id
      };
  ...
  private setDataTotales() {
    this.totalAccionesDeRestauracionActiva = this.returTotalValue(this.listOriginResources.origin_restoration);
    this.totalAccionesDeRestauracionPasiva = this.returTotalValue(this.listOriginResources.origin_manteniment);
    this.totalBuenasPracticasYSaneamiento = this.returTotalValue(this.listOriginResources.origin_practices);
  }

  private returTotalValue(data: Array<any>): number {
    let total: number;
    total = 0;
    if (data.length > 0) {
      for (let action of data) {
        total += (action.value_total);
      }
    }
  ...
  // asignar asociados al presupuesto / actividad
  ...
  public addSelectAssociated(data: any) {
    this.tasksManager.insertAssociatedPerProcess(data)
      .then(response => {
        this.montoUno = null;
        const message = {
          'tipo': 'Registrado: ',
